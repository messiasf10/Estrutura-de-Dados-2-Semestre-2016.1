...
import arvoreAVLDefinitiva.AVLNode;
...
public class HashMeioAbertaModificadaDefinitiva {
	...
	public HashMeioAbertaModificadaDefinitiva(int tamHash, int limite){
		this.TH = tamHash;
		this.LFC = limite;
		this.FB = this.FC = this.NCF = 0;		
		this.hash = new AVLArvore[this.TH];
		for(int i = 0; i < this.TH; i++)
			this.hash[i] = new AVLArvore();
	}
	...
	public void calculoFC(){
		...
		for(int i = 0; i < this.TH; i++)
			fatorCarga = (hash[i].getAlturaArvore()+1> fatorCarga) ? hash[i].getAlturaArvore()+1 : fatorCarga;
	...
	public void inserir(int valor){
		...
		for(int i=0; i<TH; i++) {
	        ...
	        if (celulaComCargaMaxima){	        		        	
	        	...
	        	AVLNode temp = hash[indice].buscar(valor);
	        	if(temp == null) {
		            ...
		            if ( getPNCF() > MUITO_CHEIA ){
		            	System.out.println("\tMAIS DA METADE DAS CELULAS DA HASHING ATINGIU O LIMITE DO FATOR DE CARGA.\n\tNECESSARIO FAZER REHASHING");
		            	this.fazerRehashing();	            	
		            }
	...
	public int calculoNCF(){
        ...
        for(int j = 0; j < this.TH; j++){	            	
        	if (hash[j].getAlturaArvore()+1 == this.LFC){
        		this.NCF += 1;
        	}
        }
        return this.NCF;
	 ...
	 */
	private boolean celulaComCargaMaxima(AVLArvore avl){
		return avl.getAlturaArvore()+1 == this.LFC;
	}
	...
	public void fazerRehashing() {
		...
		for (int i = 0; i < this.TH; i++){
			while(!hash[i].vazia()){
				temp.inserir(hash[i].valorDaRaiz());
				hash[i].deletar(hash[i].valorDaRaiz());
			}
		}
